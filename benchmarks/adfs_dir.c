...
int adfs_dir_read_buffers(struct super_block *sb, unsigned int indaddr,
...
static int adfs_dir_read(struct super_block *sb, unsigned int indaddr,
			 unsigned int size, struct adfs_dir *dir)
{
	dir->sb = sb;
	dir->bhs = dir->bh;
...
void adfs_object_fixup(struct adfs_dir *dir, struct object_info *obj)
{
	...
	if (!(obj->attr & ADFS_NDA_DIRECTORY) && ADFS_SB(dir->sb)->s_ftsuffix) {
		unsigned short int filetype = adfs_filetype(obj->loadaddr);
...
static int __adfs_compare(const unsigned char *qstr, unsigned int qlen,
