...
#define IS_LAST_ENTRY(entry) (*(unsigned *)(entry) == 0)
...
bad_block:
	...
	if (header) {
		...
		if (header->h_refcount == cpu_to_le32(1)) {
			unsigned hash = le32_to_cpu(header->h_hash);
...
retry_ref:
	...
	if (HDR(bh)->h_refcount == cpu_to_le32(1)) {
		unsigned hash = le32_to_cpu(HDR(bh)->h_hash);
		struct mb_cache_entry *oe;
		...
		struct ext2_xattr_header *header)
{
	...
	if (header) {
		...
		if (new_bh) {
			...
			struct ext2_fsblk_t goal = ext2_group_first_block_no(sb,
						EXT2_I(inode)->i_block_group);
			unsigned long count = 1;
			struct ext2_fsblk_t block = ext2_new_blocks(inode, goal,
						&count, &error,
					 ...
					 struct ext2_xattr_entry *entry)
{
	...
	if (entry->e_value_block == 0 && entry->e_value_size != 0) {
		unsigned *value = (unsigned *)((char *)header +
