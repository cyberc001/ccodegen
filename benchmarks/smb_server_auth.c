			    ...
			    char *ntlmv2_hash, char *dname)
{
	...
	hmac_md5_update(&ctx, (const unsigned char *)uniname, UNICODE_LEN(conv_len));
	...
	conv_len = smb_strtoUTF16((short *)domain, dname, len,
				  conn->local_nls);
	if (conv_len < 0 || conv_len > len) {
		ret = -EINVAL;
		goto out;
	...
	hmac_md5_update(&ctx, (const unsigned char *)domain, UNICODE_LEN(conv_len));
		      ...
		      char *cryptkey)
{
	...
	hmac_md5_update(&ctx, (const unsigned char *)&ntlmv2->blob_signature, blen);
				   ...
				   struct ksmbd_session *sess)
{
	...
	if (blob_len < (unsigned long long)dn_off + dn_len || blob_len < (unsigned long long)nt_off + nt_len ||
	...
	if (conn->ntlmssp.client_flags & NTLMSSP_NEGOTIATE_KEY_XCH) {
		...
		if (blob_len < (unsigned long long)sess_key_off + sess_key_len)
			return -EINVAL;
				   ...
				   struct ksmbd_conn *conn)
{
	...
	conv_len = smb_strtoUTF16((short *)name, ksmbd_netbios_name(), len,
				  conn->local_nls);
	if (conv_len < 0 || conv_len > len) {
		kfree(name);
		return -EINVAL;
	...
	target_name = (unsigned char *)chgblob + blob_off;
...
static void generate_key(struct ksmbd_conn *conn, struct ksmbd_session *sess,
			 struct kvec label, struct kvec context, unsigned char *key,
			 unsigned int key_size)
{
	unsigned char zero = 0x0;
	unsigned char i[4] = {0, 0, 0, 1};
	unsigned char L128[4] = {0, 0, 0, 128};
	unsigned char L256[4] = {0, 0, 1, 0};
...
static int ksmbd_get_encryption_key(struct ksmbd_work *work, unsigned long long ses_id,
				    int enc, unsigned char *key)
{
	struct ksmbd_session *sess;
	unsigned char *ses_enc_key;
			...
			unsigned int nvec, int enc)
{
	...
	int rc;
	struct scatterlist *sg;
	unsigned char sign[SMB2_SIGNATURE_SIZE] = {};
	unsigned char key[SMB3_ENC_DEC_KEY_SIZE];
