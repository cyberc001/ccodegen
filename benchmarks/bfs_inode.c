...
static struct bfs_inode *find_inode(struct super_block *sb, unsigned short ino, struct buffer_head **p)
{
...
static int bfs_write_inode(struct inode *inode, struct writeback_control *wbc)
{
	...
	unsigned int ino = (unsigned short)inode->i_ino;
...
static int bfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	struct bfs_sb_info *info = BFS_SB(s);
	unsigned long longid = huge_encode_dev(s->s_bdev->bd_dev);
