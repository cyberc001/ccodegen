...
static int squashfs_bio_read_cached(struct bio *fullbio,
		struct address_space *cache_mapping, unsigned long long index, int length,
		unsigned long long read_start, unsigned long long read_end, int page_count)
{
...
static int squashfs_bio_read(struct super_block *sb, unsigned long long index, int length,
			     struct bio **biop, int *block_offset)
{
	struct squashfs_sb_info *msblk = sb->s_fs_info;
	struct address_space *cache_mapping = msblk->cache_mapping;
	const unsigned long long read_start = round_down(index, msblk->devblksize);
	const long block = read_start >> msblk->devblksize_log2;
	const unsigned long long read_end = round_up(index + length, msblk->devblksize);
	const long block_end = read_end >> msblk->devblksize_log2;
	...
	for (i = 0; i < page_count; ++i) {
		...
		long index = (read_start >> PAGE_SHIFT) + i;
		struct page *page;
 ...
 * algorithms).
 */
int squashfs_read_data(struct super_block *sb, unsigned long long index, int length,
		       unsigned long long *next_index, struct squashfs_page_actor *output)
{
	struct squashfs_sb_info *msblk = sb->s_fs_info;
	struct bio *bio = NULL;
	int compressed;
