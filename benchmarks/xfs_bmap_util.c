 ...
 */
struct xfs_daddr_t
xfs_fsb_to_db(struct xfs_inode *ip, struct xfs_fsblock_t fsb)
{
...
xfs_zero_extent(
	struct xfs_inode	*ip,
	struct xfs_fsblock_t		start_fsb,
	struct xfs_off_t		count_fsb)
{
...
xfs_bmap_count_leaves(
	...
	struct xfs_filblks_t		*count)
{
	struct xfs_iext_cursor	icur;
	struct xfs_bmbt_irec	got;
	struct xfs_extnum_t		numrecs = 0;
...
xfs_bmap_count_blocks(
	...
	struct xfs_inode	*ip,
	int			whichfork,
	struct xfs_extnum_t		*nextents,
	struct xfs_filblks_t		*count)
{
	struct xfs_mount	*mp = ip->i_mount;
	struct xfs_ifork	*ifp = xfs_ifork_ptr(ip, whichfork);
	struct xfs_btree_cur	*cur;
	struct xfs_filblks_t		btblocks = 0;
	int			error;
...
xfs_getbmap_report_hole(
	...
	struct kgetbmap		*out,
	long long int			bmv_end,
	struct xfs_fileoff_t		bno,
	struct xfs_fileoff_t		end)
{
	struct kgetbmap		*p = out + bmv->bmv_entries;
...
xfs_getbmap_next_rec(
	...
	struct xfs_fileoff_t		total_end)
{
	struct xfs_fileoff_t		end = rec->br_startoff + rec->br_blockcount;
	...
	struct kgetbmap		*out)
{
	...
	long long int			bmv_end, max_len;
	struct xfs_fileoff_t		bno, first_bno;
	struct xfs_ifork	*ifp;
	struct xfs_bmbt_irec	got, rec;
	struct xfs_filblks_t		len;
	struct xfs_iext_cursor	icur;
	...
	while (!xfs_getbmap_full(bmv)) {
		...
		if (!xfs_iext_next_extent(ifp, &icur, &got)) {
			struct xfs_fileoff_t	end = XFS_B_TO_FSB(mp, XFS_ISIZE(ip));
...
xfs_bmap_punch_delalloc_range(
	struct xfs_inode	*ip,
	int			whichfork,
	struct xfs_off_t		start_byte,
	struct xfs_off_t		end_byte,
	struct xfs_zone_alloc_ctx *ac)
{
	struct xfs_mount	*mp = ip->i_mount;
	struct xfs_ifork	*ifp = xfs_ifork_ptr(ip, whichfork);
	struct xfs_fileoff_t		start_fsb = XFS_B_TO_FSBT(mp, start_byte);
	struct xfs_fileoff_t		end_fsb = XFS_B_TO_FSB(mp, end_byte);
...
{
	struct xfs_mount	*mp = ip->i_mount;
	int			found_blocks = false;
	struct xfs_fileoff_t		end_fsb;
	struct xfs_fileoff_t		last_fsb;
...
xfs_alloc_file_space(
	struct xfs_inode	*ip,
	struct xfs_off_t		offset,
	struct xfs_off_t		len)
{
	xfs_mount_t		*mp = ip->i_mount;
	struct xfs_off_t		count;
	struct xfs_filblks_t		allocatesize_fsb;
	struct xfs_extlen_t		extsz, temp;
	struct xfs_fileoff_t		startoffset_fsb;
	struct xfs_fileoff_t		endoffset_fsb;
	int			rt;
	struct xfs_trans_t		*tp;
	struct xfs_bmbt_irec_t		imaps[1], *imapp;
	int			error;
...
xfs_unmap_extent(
	struct xfs_inode	*ip,
	struct xfs_fileoff_t		startoffset_fsb,
	struct xfs_filblks_t		len_fsb,
	int			*done)
{
	struct xfs_mount	*mp = ip->i_mount;
	struct xfs_trans	*tp;
	unsigned int			resblks = XFS_DIOSTRAT_SPACE_RES(mp, 0);
	int			error;
	...
	struct xfs_off_t		len)
{
	...
	struct xfs_off_t		rounding, start, end;
	int			error;
	 ...
	 */
	rounding = max_t(struct xfs_off_t, xfs_inode_alloc_unitsize(ip), PAGE_SIZE);
	...
	struct xfs_zone_alloc_ctx *ac)
{
	...
	struct xfs_trans	*tp;
	int			error;
	struct xfs_fileoff_t		next_fsb = XFS_B_TO_FSB(mp, offset + len);
	struct xfs_fileoff_t		shift_fsb = XFS_B_TO_FSB(mp, len);
	int			done = false;
	...
	long			len)
{
	...
	struct xfs_trans	*tp;
	int			error;
	struct xfs_fileoff_t		stop_fsb = XFS_B_TO_FSB(mp, offset);
	struct xfs_fileoff_t		next_fsb = NULLFSBLOCK;
	struct xfs_fileoff_t		shift_fsb = XFS_B_TO_FSB(mp, len);
	int			done = false;
	...
	struct xfs_inode		*tip)
{
	...
	struct xfs_fileoff_t			offset_fsb;
	struct xfs_fileoff_t			end_fsb;
	struct xfs_filblks_t			count_fsb;
	int				error;
	struct xfs_filblks_t			ilen;
	struct xfs_filblks_t			rlen;
	int				nimaps;
	unsigned long long int			tip_flags2;
	...
	count_fsb = (struct xfs_filblks_t)(end_fsb - offset_fsb);
	...
	while (count_fsb) {
		...
		while (tirec.br_blockcount) {
			...
			/* Trim the extent. */
			uirec = tirec;
			uirec.br_blockcount = rlen = min_t(struct xfs_filblks_t,
					tirec.br_blockcount,
					irec.br_blockcount);
...
{
	struct xfs_filblks_t		aforkblks = 0;
	struct xfs_filblks_t		taforkblks = 0;
	struct xfs_extnum_t		junk;
	 ...
	 */
	tmp = (unsigned long long int)ip->i_nblocks;
	...
	struct xfs_swapext	*sxp)
{
	...
	if (xfs_has_rmapbt(mp)) {
		int		w = XFS_DATA_FORK;
		unsigned int	ipnext = ip->i_df.if_nextents;
		unsigned int	tipnext	= tip->i_df.if_nextents;
