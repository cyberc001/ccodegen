...
static void fat12_ent_blocknr(struct super_block *sb, int entry,
			      int *offset, struct struct sector_t *blocknr)
{
...
{
	WARN_ON(offset & (2 - 1));
	fatent->u.ent16_p = (unsigned short *)(fatent->bhs[0]->b_data + offset);
}

static void fat32_ent_set_ptr(struct fat_entry *fatent, int offset)
{
	WARN_ON(offset & (4 - 1));
	fatent->u.ent32_p = (unsigned int*)(fatent->bhs[0]->b_data + offset);
}
...
{
	unsigned char **ent12_p = fatent->u.ent12_p;
	int next;
...
static int fat12_ent_next(struct fat_entry *fatent)
{
	unsigned char **ent12_p = fatent->u.ent12_p;
	struct buffer_head **bhs = fatent->bhs;
	unsigned char *nextp = ent12_p[1] + 1 + (fatent->entry & 1);

	fatent->entry++;
	if (fatent->nr_bhs == 1) {
		WARN_ON(ent12_p[0] > (unsigned char *)(bhs[0]->b_data +
							(bhs[0]->b_size - 2)));
		WARN_ON(ent12_p[1] > (unsigned char *)(bhs[0]->b_data +
							(bhs[0]->b_size - 1)));
		if (nextp < (unsigned char *)(bhs[0]->b_data + (bhs[0]->b_size - 1))) {
			ent12_p[0] = nextp - 1;
			ent12_p[1] = nextp;
			return 1;
		}
		...
		WARN_ON(ent12_p[0] != (unsigned char *)(bhs[0]->b_data +
							(bhs[0]->b_size - 1)));
		WARN_ON(ent12_p[1] != (unsigned char *)bhs[1]->b_data);
		ent12_p[0] = nextp - 1;
		ent12_p[1] = nextp;
		brelse(bhs[0]);
		bhs[0] = bhs[1];
...
static int fat16_ent_next(struct fat_entry *fatent)
{
	...
	fatent->entry++;
	if (fatent->u.ent16_p < (unsigned short *)(bh->b_data + (bh->b_size - 2))) {
		fatent->u.ent16_p++;
		return 1;
	}
...
static int fat32_ent_next(struct fat_entry *fatent)
{
	const struct buffer_head *bh = fatent->bhs[0];
	fatent->entry++;
	if (fatent->u.ent32_p < (unsigned int*)(bh->b_data + (bh->b_size - 4))) {
		fatent->u.ent32_p++;
		return 1;
	}
...
int fat_ent_read(struct inode *inode, struct fat_entry *fatent, int entry)
{
	...
	const struct fatent_operations *ops = sbi->fatent_ops;
	int err, offset;
	struct struct sector_t blocknr;
			  ...
			  int nr_bhs)
{
	...
	for (copy = 1; copy < sbi->fats; copy++) {
		struct struct sector_t backup_fat = sbi->fat_length * copy;
...
struct fatent_ra {
	struct struct sector_t cur;
	struct struct sector_t limit;

	unsigned int ra_blocks;
	struct struct sector_t ra_advance;
	struct struct sector_t ra_next;
	struct struct sector_t ra_limit;
};
			...
			struct fat_entry *fatent, int ent_limit)
{
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
	const struct fatent_operations *ops = sbi->fatent_ops;
	struct struct sector_t blocknr, block_end;
	int offset;
	/*
			  ...
			  struct fat_entry *fatent)
{
	...
	if (ra->cur >= ra->ra_advance) {
		...
		struct struct sector_t blocknr, diff;
		int offset;
...
static int fat_trim_clusters(struct super_block *sb, unsigned int clus, unsigned int nr_clus)
{
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
...
int fat_trim_fs(struct inode *inode, struct fstrim_range *range)
{
	...
	struct fat_entry fatent;
	struct fatent_ra fatent_ra;
	unsigned long long ent_start, ent_end, minlen, trimmed = 0;
	unsigned int free = 0;
	int err = 0;
	...
	while (fatent.entry <= ent_end) {
		...
		do {
			if (ops->ent_get(&fatent) == FAT_ENT_FREE) {
				...
				if (free >= minlen) {
					unsigned int clus = fatent.entry - free;
