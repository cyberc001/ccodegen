...
static int ufs_block_to_path(struct inode *inode, struct sector_t i_block, unsigned* offsets)
{
...
static inline int grow_chain32(struct ufs_inode_info *ufsi,
			       struct buffer_head *bh, struct __fs32 *v,
			       Indirect *from, Indirect *to)
{
	...
	do {
		...
		to->key32 = *(struct __fs32 *)(to->p = v);
		for (p = from; p <= to && p->key32 == *(struct __fs32 *)p->p; p++)
			;
...
static inline int grow_chain64(struct ufs_inode_info *ufsi,
			       struct buffer_head *bh, struct __fs64 *v,
			       Indirect *from, Indirect *to)
{
	Indirect *p;
	unsigned seq;
	...
	do {
		seq = read_seqbegin(&ufsi->meta_lock);
		to->key64 = *(struct __fs64 *)(to->p = v);
		for (p = from; p <= to && p->key64 == *(struct __fs64 *)p->p; p++)
			;
	} while (read_seqretry(&ufsi->meta_lock, seq));
	return (p > to);
}
...
static unsigned long long int ufs_frag_map(struct inode *inode, unsigned offsets, int depth)
{
	struct ufs_inode_info *ufsi = UFS_I(inode);
	struct super_block *sb = inode->i_sb;
	struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
	unsigned long long int mask = (unsigned long long int) uspi->s_apbmask>>uspi->s_fpbshift;
...
again:
	...
	while (--depth) {
		...
		if (!bh)
			goto no_block;
		ptr = (struct __fs32 *)bh->b_data + (n & mask);
...
ufs2:
	...
	while (--depth) {
		...
		if (!bh)
			goto no_block;
		ptr = (struct __fs64 *)bh->b_data + (n & mask);
 ...
 */
static int
ufs_extend_tail(struct inode *inode, unsigned long long int writes_to,
 ...
 */
static unsigned long long int ufs_inode_getfrag(struct inode *inode, unsigned index,
		  struct sector_t new_fragment, int *err,
		  int *new, struct folio *locked_folio)
{
	struct ufs_inode_info *ufsi = UFS_I(inode);
	struct super_block *sb = inode->i_sb;
	struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
	unsigned long long int tmp, goal, lastfrag;
 ...
 */
static unsigned long long int ufs_inode_getblock(struct inode *inode, unsigned long long int ind_block,
		unsigned index, struct sector_t new_fragment, int *err,
		int *new, struct folio *locked_folio)
{
	struct super_block *sb = inode->i_sb;
	struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
	int shift = uspi->s_apbshift - uspi->s_fpbshift;
	unsigned long long int tmp = 0, goal;
	struct buffer_head *bh;
	void *p;
	...
	if (uspi->fs_magic == UFS2_MAGIC)
		p = (struct __fs64 *)bh->b_data + index;
	else
		p = (struct __fs32 *)bh->b_data + index;
	...
	if (index && (uspi->fs_magic == UFS2_MAGIC ?
		      (tmp = fs64_to_cpu(sb, ((struct __fs64 *)bh->b_data)[index-1])) :
		      (tmp = fs32_to_cpu(sb, ((struct __fs32 *)bh->b_data)[index-1]))))
		goal = tmp + uspi->s_fpb;
	else
...
static int ufs_getfrag_block(struct inode *inode, struct sector_t fragment, struct buffer_head *bh_result, int create)
{
	struct super_block *sb = inode->i_sb;
	struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
	int err = 0, new = 0;
	...
	unsigned long long int phys64 = 0;
...
int ufs_prepare_chunk(struct folio *folio, long pos, unsigned len)
{
...
static void ufs_write_failed(struct address_space *mapping, long to)
{
...
static int ufs_write_end(const struct kiocb *iocb,
			 struct address_space *mapping,
			 long pos, unsigned len, unsigned copied,
...
static struct sector_t ufs_bmap(struct address_space *mapping, struct sector_t block)
{
...
static inline void free_data(struct to_free *ctx, unsigned long long int from, unsigned count)
{
...
static void ufs_trunc_direct(struct inode *inode)
{
	...
	if (old_slot == new_slot) { // old_tail > 0
		...
		unsigned long long int tmp = ufs_data_ptr_to_cpu(sb, p);
		if (!tmp)
...
static void free_full_branch(struct inode *inode, unsigned long long int ind_block, int depth)
{
	struct super_block *sb = inode->i_sb;
	struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
	...
	if (--depth) {
		for (i = 0; i < uspi->s_apb; i++) {
			...
			unsigned long long int block = ufs_data_ptr_to_cpu(sb, p);
			if (block)
...
static int ufs_alloc_lastblock(struct inode *inode, long size)
{
	int err = 0;
	struct super_block *sb = inode->i_sb;
	...
	struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
	unsigned i, end;
	struct sector_t lastfrag;
	struct folio *folio;
	struct buffer_head *bh;
	unsigned long long int phys64;
...
static void ufs_truncate_blocks(struct inode *inode)
{
	...
	if (inode->i_size) {
		struct sector_t last = (inode->i_size - 1) >> uspi->s_bshift;
...
static int ufs_truncate(struct inode *inode, long size)
{
	int err = 0;
