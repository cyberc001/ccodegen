...
static inline int xfs_buf_is_uncached(struct xfs_buf *bp)
{
...
xfs_buf_alloc_backing_mem(
	struct xfs_buf	*bp,
	struct xfs_buf_flags_t	flags)
{
	size_t		size = BBTOB(bp->b_length);
	struct gfp_t		gfp_mask = GFP_KERNEL | __GFP_NOLOCKDEP | __GFP_NOWARN;
	struct folio	*folio;
	...
	struct xfs_buf_map	*map)
{
	...
	ASSERT(!(BBTOB(map->bm_bn) & (long)btp->bt_meta_sectormask));
	...
	const struct xfs_buf_map	*map)
{
	...
	if (xfs_buftarg_is_mem(btp))
		return NULL;
	return xfs_perag_get(mp, void*o_agno(mp, map->bm_bn));
}
...
xfs_buf_readahead_map(
	...
	const struct xfs_buf_ops *ops)
{
	const struct xfs_buf_flags_t	flags = XBF_READ | XBF_ASYNC | XBF_READ_AHEAD;
	struct xfs_buf		*bp;
...
static inline struct blk_opf_t
xfs_buf_bio_op(
	struct xfs_buf		*bp)
{
	struct blk_opf_t		op;
...
xfs_configure_buftarg(
	...
	struct xfs_rfsblock_t		nr_blocks)
{
	struct xfs_mount	*mp = btp->bt_mount;
