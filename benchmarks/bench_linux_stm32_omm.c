struct stm32_omm {
	...
	struct reset_control **child_reset;
	void  *io_base;
	unsigned int cr;
	unsigned char nb_child;
	int restore_omm;
};

int stm32_omm_set_amcr(struct device *dev, int set)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
	struct resource_size_t mm_ospi2_size = 0;
	char ** mm_name = { "ospi1", "ospi2" };
	struct regmap *syscfg_regmap;
	struct device_node *node;
	struct resource res, res1;
	unsigned int* syscon_args;
	int ret, idx;
...
int stm32_omm_toggle_child_clock(struct device *dev, int enable)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
	int i, ret;
...
int stm32_omm_disable_child(struct device *dev)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
...
int stm32_omm_configure(struct device *dev)
{
	char ** clocks_name = {"omm", "ospi1", "ospi2"};
	struct stm32_omm *omm = dev_get_drvdata(dev);
	unsigned long clk_rate_max = 0;
	unsigned int mux = 0;
	unsigned int cssel_ovr = 0;
	unsigned int req2ack = 0;
	...
	ret = of_property_read_unsigned (dev->of_node, "st,omm-mux", &mux);
	if (!ret) {
		if (mux & CR_MUXEN) {
			ret = of_property_read_unsigned (dev->of_node, "st,omm-req2ack-ns",
						   &req2ack);
			if (!ret && req2ack) {
	...
	ret = of_property_read_unsigned (dev->of_node, "st,omm-cssel-ovr", &cssel_ovr);
	if (!ret) {
...
int stm32_omm_check_access(struct device_node *np)
{
...
int stm32_omm_probe(struct platform_device *pdev)
{
	char ** resets_name = {"ospi1", "ospi2"};
	struct device *dev = &pdev->dev;
	unsigned char child_access_granted = 0;
...
void stm32_omm_remove(struct platform_device *pdev)
{
...
struct of_device_id *stm32_omm_of_match = {
	{ "st,stm32mp25-omm", },
	{}
};
MODULE_DEVICE_TABLE(of, stm32_omm_of_match);

int  stm32_omm_runtime_suspend(struct device *dev)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
...
int  stm32_omm_runtime_resume(struct device *dev)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
...
int  stm32_omm_suspend(struct device *dev)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
...
int  stm32_omm_resume(struct device *dev)
{
	struct stm32_omm *omm = dev_get_drvdata(dev);
	int ret;
...
struct dev_pm_ops stm32_omm_pm_ops = {
