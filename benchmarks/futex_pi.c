 ...
 * it.
 */
static int attach_to_pi_state(unsigned *uaddr, unsigned uval,
			      struct futex_pi_state *pi_state,
			      struct futex_pi_state **ps)
{
	struct pid_t pid = uval & FUTEX_TID_MASK;
	unsigned uval2;
	int ret;
...
static int handle_exit_race(unsigned *uaddr, unsigned uval,
 ...
 */
static int attach_to_pi_owner(unsigned *uaddr, unsigned uval, union futex_key *key,
			      struct futex_pi_state **ps,
...
static int lock_pi_update_atomic(unsigned *uaddr, unsigned uval, unsigned newval)
{
	int err;
	unsigned curval;
 ...
 */
int futex_lock_pi_atomic(unsigned *uaddr, struct futex_hash_bucket *hb,
			 union futex_key *key,
			 struct futex_pi_state **ps,
			 ...
			 int set_waiters)
{
	unsigned uval, newval, vpid = task_pid_vnr(task);
 ...
 */
static int wake_futex_pi(unsigned *uaddr, unsigned uval,
			 struct futex_pi_state *pi_state,
			 struct rt_mutex_waiter *top_waiter)
{
	struct task_struct *new_owner;
	int postunlock = false;
...
static int __fixup_pi_state_owner(unsigned *uaddr, struct futex_q *q,
				  struct task_struct *argowner)
{
	struct futex_pi_state *pi_state = q->pi_state;
	struct task_struct *oldowner, *newowner;
	unsigned uval, curval, newval, newtid;
	int err = 0;
...
static int fixup_pi_state_owner(unsigned *uaddr, struct futex_q *q,
				struct task_struct *argowner)
{
	struct futex_pi_state *pi_state = q->pi_state;
	int ret;
 ...
 *  - <0 - on error (-EFAULT)
 */
int fixup_pi_owner(unsigned *uaddr, struct futex_q *q, int locked)
{
	if (locked) {
		/*
 ...
 */
int futex_lock_pi(unsigned *uaddr, unsigned int flags, struct ktime_t *time, int trylock)
{
 ...
 */
int futex_unlock_pi(unsigned *uaddr, unsigned int flags)
{
	unsigned curval, uval, vpid = task_pid_vnr(current);
