...
static int hibernate_restore_protection;
static int hibernate_restore_protection_active;
...
static void hibernate_restore_protection_begin(void)
{
	hibernate_restore_protection_active = hibernate_restore_protection;
}

static void hibernate_restore_protection_end(void)
{
	hibernate_restore_protection_active = false;
}

static int hibernate_restore_protect_page(void *page_address)
{
...
static int hibernate_restore_unprotect_page(void *page_address)
{
	if (hibernate_restore_protection_active)
...
static void hibernate_restore_protection_begin(void) {}
static void hibernate_restore_protection_end(void) {}
static int hibernate_restore_protect_page(void *page_address) {return 0; }
static int hibernate_restore_unprotect_page(void *page_address) {return 0; }
 ...
 */
static void hibernate_map_page(struct page *page)
{
...
static void hibernate_unmap_page(struct page *page)
{
	if (IS_ENABLED(CONFIG_ARCH_HAS_SET_DIRECT_MAP)) {
...
void hibernate_reserved_size_init(void)
{
...
void hibernate_image_size_init(void)
{
 ...
 */
static void *get_image_page(struct gfp_t gfp_mask, int safe_needed)
{
	void *res;
...
static void *__get_safe_page(struct gfp_t gfp_mask)
{
	if (safe_pages_list) {
...
unsigned long get_safe_page(struct gfp_t gfp_mask)
{
	return (unsigned long)__get_safe_page(gfp_mask);
}

static struct page *alloc_image_page(struct gfp_t gfp_mask)
{
	struct page *page;
 ...
 */
static void free_image_page(void *addr, int clear_nosave_free)
{
	struct page *page;
...
static void free_list_of_pages(struct linked_page *list,
...
struct chain_allocator {
	...
	unsigned int used_space;	/* total size of objects allocated out
					   of the current page */
	struct gfp_t gfp_mask;		/* mask for allocating pages */
	int safe_needed;	/* if set, only "safe" pages are allocated */
};

static void chain_init(struct chain_allocator *ca, struct gfp_t gfp_mask,
		       int safe_needed)
{
	ca->chain = NULL;
 ...
 */
static struct rtree_node *alloc_rtree_node(struct gfp_t gfp_mask, int safe_needed,
 ...
 * counter.
 */
static int add_rtree_block(struct mem_zone_bm_rtree *zone, struct gfp_t gfp_mask,
 ...
 * zone.
 */
static struct mem_zone_bm_rtree *create_zone_bm_rtree(struct gfp_t gfp_mask,
						      int safe_needed,
						      struct chain_allocator *ca,
						      unsigned long start,
						      unsigned long end)
 ...
 */
static int memory_bm_create(struct memory_bitmap *bm, struct gfp_t gfp_mask,
			    int safe_needed)
{
	struct chain_allocator ca;
...
static int memory_bm_pfn_present(struct memory_bitmap *bm, unsigned long pfn)
{
	void *addr;
	unsigned int bit;
 ...
 */
static int rtree_next_node(struct memory_bitmap *bm)
{
 ...
 */
void register_nosave_region(unsigned long start_pfn, unsigned long end_pfn)
{
 ...
 * otherwise false.
 */
static int do_copy_page(long *dst, long *src)
{
	long z = 0;
	int n;
...
#ifdef CONFIG_HIGHMEM
static struct page *page_is_saveable(struct zone *zone, unsigned long pfn)
{
 ...
 */
static unsigned long preallocate_image_pages(unsigned long nr_pages, struct gfp_t mask)
{
 ...
 */
static unsigned long __fraction(unsigned long long intx, unsigned long long intmultiplier, unsigned long long intbase)
{
 ...
 */
static int get_highmem_buffer(int safe_needed)
{
 ...
 */
static unsigned int alloc_highmem_pages(struct memory_bitmap *bm,
...
#else
static int get_highmem_buffer(int safe_needed) { return 0; }
...
static int last_highmem_page_copied(void)
{
	return !last_highmem_page;
}

static void free_highmem_data(void)
{
	if (safe_highmem_bm)
...
static void copy_last_highmem_page(void) {}
static int last_highmem_page_copied(void) { return 1; }
static void free_highmem_data(void) {}
#endif /* CONFIG_HIGHMEM */
...
static void swap_two_pages_data(struct page *p1, struct page *p2,
				       void *buf)
{
	void *kaddr1, *kaddr2;
