...
static int ext4_fc_disabled(struct super_block *sb)
{
 ...
 */
void ext4_fc_mark_ineligible(struct super_block *sb, int reason, struct handle_t *handle)
{
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct tid_t tid;
	int has_transaction = true;
	int is_ineligible;
	int alloc_ctx;
...
static int ext4_fc_track_template(
	struct handle_t *handle, struct inode *inode,
	void* __fc_track_fn,
...
static int __track_dentry_update(struct handle_t *handle, struct inode *inode,
				 void *arg, int update)
{
...
void __ext4_fc_track_unlink(struct handle_t *handle,
...
void ext4_fc_track_unlink(struct handle_t *handle, struct dentry *dentry)
{
...
void __ext4_fc_track_link(struct handle_t *handle,
	struct inode *inode, struct dentry *dentry)
{
	struct __track_dentry_update_args args;
	int ret;
...
void ext4_fc_track_link(struct handle_t *handle, struct dentry *dentry)
{
	struct inode *inode = d_inode(dentry);
...
void __ext4_fc_track_create(struct handle_t *handle, struct inode *inode,
			  struct dentry *dentry)
{
	struct __track_dentry_update_args args;
	int ret;
...
void ext4_fc_track_create(struct handle_t *handle, struct dentry *dentry)
{
	struct inode *inode = d_inode(dentry);
...
static int __track_inode(struct handle_t *handle, struct inode *inode, void *arg,
			 int update)
{
	if (update)
		return -EEXIST;
...
void ext4_fc_track_inode(struct handle_t *handle, struct inode *inode)
{
	struct ext4_inode_info *ei = EXT4_I(inode);
	wait_queue_head_t *wq;
	int ret;
...
struct __track_range_args {
	struct ext4_lblk_t start, end;
};

/* __track_fn for tracking data updates */
static int __track_range(struct handle_t *handle, struct inode *inode, void *arg,
			 int update)
{
	struct ext4_inode_info *ei = EXT4_I(inode);
	struct ext4_lblk_t oldstart;
...
void ext4_fc_track_range(struct handle_t *handle, struct inode *inode, struct ext4_lblk_t start,
			 struct ext4_lblk_t end)
{
...
static void ext4_fc_submit_bh(struct super_block *sb, int is_tail)
{
	struct blk_opf_t write_flags = JBD2_JOURNAL_REQ_FLAGS;
 ...
 * we added.
 */
static unsigned char *ext4_fc_reserve_space(struct super_block *sb, int len, unsigned *crc)
{
 ...
 */
static int ext4_fc_write_tail(struct super_block *sb, unsigned crc)
{
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_fc_tl tl;
	...
	crc = ext4_chksum(crc, sbi->s_fc_bh->b_data,
			  dst - (unsigned char *)sbi->s_fc_bh->b_data);
 ...
 */
static int ext4_fc_add_tlv(struct super_block *sb, unsigned short tag, unsigned short len, unsigned char *val,
			   unsigned *crc)
{
	struct ext4_fc_tl tl;
	unsigned char *dst;
...
static int ext4_fc_add_dentry_tlv(struct super_block *sb, unsigned *crc,
				   struct ext4_fc_dentry_update *fc_dentry)
{
	struct ext4_fc_dentry_info fcd;
	struct ext4_fc_tl tl;
	int dlen = fc_dentry->fcd_name.name.len;
	unsigned char *dst = ext4_fc_reserve_space(sb,
 ...
 */
static int ext4_fc_write_inode(struct inode *inode, unsigned *crc)
{
	struct ext4_inode_info *ei = EXT4_I(inode);
	...
	dst += sizeof(fc_inode);
	memcpy(dst, (unsigned char *)ext4_raw_inode(&iloc), inode_len);
	ret = 0;
err:
	brelse(iloc.bh);
	return ret;
 ...
 */
static int ext4_fc_write_inode_data(struct inode *inode, unsigned *crc)
{
	struct ext4_lblk_t old_blk_size, cur_lblk_off, new_blk_size;
	struct ext4_inode_info *ei = EXT4_I(inode);
	...
	while (cur_lblk_off <= new_blk_size) {
		...
		if (ret == 0) {
			...
			if (!ext4_fc_add_tlv(inode->i_sb, EXT4_FC_TAG_DEL_RANGE,
					    sizeof(lrange), (unsigned char *)&lrange, crc))
				return -ENOSPC;
		} else {
			...
			if (!ext4_fc_add_tlv(inode->i_sb, EXT4_FC_TAG_ADD_RANGE,
					    sizeof(fc_ext), (unsigned char *)&fc_ext, crc))
				return -ENOSPC;
		}
...
static int ext4_fc_perform_commit(struct journal_t *journal)
{
...
static void ext4_fc_update_stats(struct super_block *sb, int status,
				 unsigned long long commit_time, int nblks, struct tid_t commit_tid)
{
 ...
 */
int ext4_fc_commit(struct journal_t *journal, struct tid_t commit_tid)
{
	struct super_block *sb = journal->j_private;
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	...
	struct ktime_t start_time, commit_time;
 ...
 */
static void ext4_fc_cleanup(struct journal_t *journal, int full, struct tid_t tid)
{
	struct super_block *sb = journal->j_private;
	struct ext4_sb_info *sbi = EXT4_SB(sb);
...
struct ext4_fc_tl_mem {
	unsigned short fc_tag;
	unsigned short fc_len;
};

static inline void tl_to_darg(struct dentry_info_args *darg,
			      struct ext4_fc_tl_mem *tl, unsigned char *val)
{
	struct ext4_fc_dentry_info fcd;
...
static inline void ext4_fc_get_tl(struct ext4_fc_tl_mem *tl, unsigned char *val)
{
				...
				struct ext4_fc_tl_mem *tl, unsigned char *val)
{
	...
	memcpy((unsigned char *)raw_inode + off_gen, (unsigned char *)raw_fc_inode + off_gen,
	       inode_len - off_gen);
...
int ext4_fc_record_regions(struct super_block *sb, int ino,
		struct ext4_lblk_t lblk, struct ext4_fsblk_t pblk, int len, int replay)
{
	struct ext4_fc_replay_state *state;
				    ...
				    struct ext4_fc_tl_mem *tl, unsigned char *val)
{
	...
	struct inode *inode;
	struct ext4_lblk_t start, cur;
	int remaining, len;
	struct ext4_fsblk_t start_pblk;
	struct ext4_map_blocks map;
			 ...
			 struct ext4_fc_tl_mem *tl, unsigned char *val)
{
	...
	struct ext4_fc_del_range lrange;
	struct ext4_map_blocks map;
	struct ext4_lblk_t cur, remaining;
	int ret;
 ...
 */
int ext4_fc_replay_check_excluded(struct super_block *sb, struct ext4_fsblk_t blk)
{
	int i;
	struct ext4_fc_replay_state *state;
...
static int ext4_fc_value_len_isvalid(struct ext4_sb_info *sbi,
				      int tag, int len)
{
	switch (tag) {
 ...
 */
static int ext4_fc_replay_scan(struct journal_t *journal,
				struct buffer_head *bh, int off,
				struct tid_t expected_tid)
{
	struct super_block *sb = journal->j_private;
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_fc_replay_state *state;
	...
	struct ext4_fc_tail tail;
	unsigned char *start, *end, *cur, *val;
	struct ext4_fc_head head;
	struct ext4_extent *ex;

	state = &sbi->s_fc_replay_state;

	start = (unsigned char *)bh->b_data;
 ...
 */
static int ext4_fc_replay(struct journal_t *journal, struct buffer_head *bh,
				enum passtype pass, int off, struct tid_t expected_tid)
{
	struct super_block *sb = journal->j_private;
	struct ext4_sb_info *sbi = EXT4_SB(sb);
	struct ext4_fc_tl_mem tl;
...
void ext4_fc_init(struct super_block *sb, struct journal_t *journal)
{
	/*
...
int ext4_fc_init_dentry_cache(void)
{
