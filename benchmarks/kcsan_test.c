...
static int report_available(void)
{
...
static int __report_matches(const struct expect_report *r)
{
	const int is_assert = (r->access[0].type | r->access[1].type) & KCSAN_ACCESS_ASSERT;
	int ret = false;
	unsigned long flags;
			...
			(ty & KCSAN_ACCESS_ASSERT) ?
				      ...
				      ((ty & KCSAN_ACCESS_WRITE) ?
					       ((ty & KCSAN_ACCESS_COMPOUND) ?
							...
							"write") :
					       "read");
		const int is_atomic = (ty & KCSAN_ACCESS_ATOMIC);
		const int is_scoped = (ty & KCSAN_ACCESS_SCOPED);
		const char *access_type_aux =
...
const 
static const struct expect_report *
...
static int report_matches_any_reordered(struct expect_report *r)
{
...
static long test_data_racy;
DEFINE_SEQLOCK(test_seqlock);
DEFINE_SPINLOCK(test_spinlock);
...
static void sink_value(long v) { WRITE_ONCE(test_sink, v); }

/*
 * Generates a delay and some accesses that enter the runtime but do not produce
 * data races.
 */
static void test_delay(int iter)
{
	while (iter--)
		sink_value(READ_ONCE(test_sink));
}

static void test_kernel_read(void) { sink_value(test_var); }
...
static void test_kernel_write_nochange(void) { test_var = 42; }

/* Suffixed by value-change exception filter. */
static void test_kernel_write_nochange_rcu(void) { test_var = 42; }

static void test_kernel_read_atomic(void)
{
	sink_value(READ_ONCE(test_var));
}

static void test_kernel_write_atomic(void)
{
	WRITE_ONCE(test_var, READ_ONCE_NOCHECK(test_sink) + 1);
}

static void test_kernel_atomic_rmw(void)
{
...
static void test_kernel_write_uninstrumented(void) { test_var++; }

static void test_kernel_data_race(void) { data_race(test_var++); }

static void test_kernel_data_racy_qualifier(void) { test_data_racy++; }

static void test_kernel_assert_writer(void)
{
	ASSERT_EXCLUSIVE_WRITER(test_var);
}

static void test_kernel_assert_access(void)
{
...
static void test_kernel_change_bits(void)
{
...
static void test_kernel_assert_bits_change(void)
{
	ASSERT_EXCLUSIVE_BITS(test_var, TEST_CHANGE_BITS);
}

static void test_kernel_assert_bits_nochange(void)
{
	ASSERT_EXCLUSIVE_BITS(test_var, ~TEST_CHANGE_BITS);
}
 ...
 */
static void test_enter_scope(void)
{
	int x = 0;
...
static void test_kernel_assert_writer_scoped(void)
{
	ASSERT_EXCLUSIVE_WRITER_SCOPED(test_var);
	test_enter_scope();
}

static void test_kernel_assert_access_scoped(void)
{
	ASSERT_EXCLUSIVE_ACCESS_SCOPED(test_var);
	test_enter_scope();
}

static void test_kernel_rmw_array(void)
{
	int i;
...
static void test_kernel_write_struct(void)
{
...
static void test_kernel_write_struct_part(void)
{
	test_struct.val[3] = 42;
}

static void test_kernel_read_struct_zero_size(void)
{
	kcsan_check_read(&test_struct.val[3], 0);
}

static void test_kernel_jiffies_reader(void)
{
	sink_value((long)jiffies);
}

static void test_kernel_seqlock_reader(void)
{
	unsigned int seq;
...
static void test_kernel_seqlock_writer(void)
{
	unsigned long flags;
...
static void test_kernel_atomic_builtins(void)
{
	/*
...
static void test_kernel_xor_1bit(void)
{
...
#define TEST_KERNEL_LOCKED(name, acquire, release)		\
	static void test_kernel_##name(void)		\
	{							\
 ...
 * exhaustive on struct atomic_t operations. Unlike the selftest, also checks for
...
#endif
	struct spinlock_t arch_spinlock = __ARCH_SPIN_LOCK_UNLOCKED;
	struct atomic_t dummy = ATOMIC_INIT(0);
...
static void test_basic(struct kunit *test)
{
	...
	int match_expect = false;
	int match_never = false;
...
static void test_assert_exclusive_access_writer(struct kunit *test)
{
	...
	int match_expect_access_writer = false;
	int match_expect_access_access = false;
	int match_never = false;
...
static void test_assert_exclusive_writer_scoped(struct kunit *test)
{
	...
	int match_expect_start = false;
	int match_expect_inscope = false;
...
static void test_atomic_builtins(struct kunit *test)
{
	...
	do {
		...
		__atomic_store_n(&test_var, 42, __ATOMIC_RELAXED);
		KUNIT_EXPECT_EQ(test, 42, __atomic_load_n(&test_var, __ATOMIC_RELAXED));

		KUNIT_EXPECT_EQ(test, 42, __atomic_exchange_n(&test_var, 20, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, 20, test_var);

		tmp = 20;
		KUNIT_EXPECT_TRUE(test, __atomic_compare_exchange_n(&test_var, &tmp, 30,
								    0, __ATOMIC_RELAXED,
								    __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, tmp, 20);
		KUNIT_EXPECT_EQ(test, test_var, 30);
		KUNIT_EXPECT_FALSE(test, __atomic_compare_exchange_n(&test_var, &tmp, 40,
								     1, __ATOMIC_RELAXED,
								     __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, tmp, 30);
		KUNIT_EXPECT_EQ(test, test_var, 30);

		KUNIT_EXPECT_EQ(test, 30, __atomic_fetch_add(&test_var, 1, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, 31, __atomic_fetch_sub(&test_var, 1, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, 30, __atomic_fetch_and(&test_var, 0xf, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, 14, __atomic_fetch_xor(&test_var, 0xf, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, 1, __atomic_fetch_or(&test_var, 0xf0, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, 241, __atomic_fetch_nand(&test_var, 0xf, __ATOMIC_RELAXED));
		KUNIT_EXPECT_EQ(test, -2, test_var);
...
{
	static struct atomic_t cnt = ATOMIC_INIT(0);
	unsigned int idx;
