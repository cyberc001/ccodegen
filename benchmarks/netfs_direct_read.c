...
static int netfs_dispatch_unbuffered_reads(struct netfs_io_request *rreq)
{
	...
	unsigned long long size = rreq->len;
	int ret = 0;
 ...
 * local disk cache.
 */
static unsigned long long netfs_unbuffered_read(struct netfs_io_request *rreq, int sync)
{
 ...
 */
unsigned long long netfs_unbuffered_read_iter_locked(struct kiocb *iocb, struct iov_iter *iter)
{
	struct netfs_io_request *rreq;
	unsigned long long ret;
	size_t orig_count = iov_iter_count(iter);
	int sync = is_sync_kiocb(iocb);
 ...
 * output buffer.  No use is made of the pagecache.
 */
unsigned long long netfs_unbuffered_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
