...
struct iomap_folio_state {
	...
	struct atomic_t		write_bytes_pending;
...
static inline int ifs_is_fully_uptodate(struct folio *folio,
...
static int ifs_set_range_uptodate(struct folio *folio,
...
static unsigned ifs_find_dirty_range(struct folio *folio,
		struct iomap_folio_state *ifs, unsigned long long *range_start, unsigned long long range_end)
{
	struct inode *inode = folio->mapping->host;
	unsigned start_blk =
...
static unsigned iomap_find_dirty_range(struct folio *folio, unsigned long long *range_start,
 ...
 */
static size_t iomap_bytes_to_truncate(long end_pos, unsigned block_bits,
...
static void iomap_adjust_read_range(struct inode *inode, struct folio *folio,
		long *pos, long length, size_t *offp, size_t *lenp)
{
	struct iomap_folio_state *ifs = folio->private;
	long orig_pos = *pos;
	long isize = i_size_read(inode);
...
static inline int iomap_block_needs_zeroing(const struct iomap_iter *iter,
		long pos)
{
 ...
 */
int iomap_is_partially_uptodate(struct folio *folio, size_t from, size_t count)
{
	struct iomap_folio_state *ifs = folio->private;
	struct inode *inode = folio->mapping->host;
	unsigned first, last;
 ...
 */
struct folio *iomap_get_folio(struct iomap_iter *iter, long pos, size_t len)
{
	struct fgf_t fgp = FGP_WRITEBEGIN | FGP_NOFS;
...
int iomap_release_folio(struct folio *folio, struct gfp_t gfp_flags)
{
...
int iomap_dirty_folio(struct address_space *mapping, struct folio *folio)
{
...
static void
iomap_write_failed(struct inode *inode, long pos, unsigned len)
{
	long i_size = i_size_read(inode);
		...
		struct folio *folio)
{
	struct iomap_folio_state *ifs;
	long pos = iter->pos;
	long block_size = i_blocksize(iter->inode);
	long block_start = round_down(pos, block_size);
	long block_end = round_up(pos + len, block_size);
...
static long iomap_trim_folio_range(struct iomap_iter *iter,
		struct folio *folio, size_t *offset, unsigned long long *bytes)
{
	long pos = iter->pos;
...
static int iomap_write_begin(struct iomap_iter *iter,
		...
		size_t *poffset, unsigned long long *plen)
{
	const struct iomap *srcmap = iomap_iter_srcmap(iter);
	long pos;
	unsigned long long len = min_t(u64, SIZE_MAX, iomap_length(iter));
	struct folio *folio;
	int status = 0;
	...
	*plen = 0;
	...
	if (write_ops && write_ops->iomap_valid) {
		int iomap_valid = write_ops->iomap_valid(iter->inode,
							 &iter->iomap);
		if (!iomap_valid) {
			iter->iomap.flags |= IOMAP_F_STALE;
			status = 0;
...
static int __iomap_write_end(struct inode *inode, long pos, size_t len,
...
static int iomap_write_end_inline(const struct iomap_iter *iter,
		struct folio *folio, long pos, size_t copied)
{
	const struct iomap *iomap = &iter->iomap;
	void *addr;
		...
		const struct iomap_write_ops *write_ops)
{
	...
	do {
		...
		unsigned long long bytes;		/* Bytes to write to folio */
		size_t copied;		/* Bytes copied from user */
		unsigned long long written;		/* Bytes have been written */
		long pos;
...
static void iomap_write_delalloc_ifs_punch(struct inode *inode,
		struct folio *folio, long start_byte, long end_byte,
		struct iomap *iomap, struct iomap_punch_t punch)
{
	unsigned int first_blk, last_blk;
	long last_byte;
	unsigned char blkbits = inode->i_blkbits;
	struct iomap_folio_state *ifs;
...
static void iomap_write_delalloc_punch(struct inode *inode, struct folio *folio,
		long *punch_start_byte, long start_byte, long end_byte,
		struct iomap *iomap, struct iomap_punch_t punch)
{
	if (!folio_test_dirty(folio))
		return;
 ...
 */
void iomap_write_delalloc_release(struct inode *inode, long start_byte,
		long end_byte, unsigned flags, struct iomap *iomap,
		struct iomap_punch_t punch)
{
	long punch_start_byte = start_byte;
	long scan_end_byte = min(i_size_read(inode), end_byte);
		...
		const struct iomap_write_ops *write_ops)
{
	...
	unsigned long long bytes = iomap_length(iter);
	int status;
...
int
iomap_file_unshare(struct inode *inode, long pos, long len,
		const struct iomap_ops *ops,
		const struct iomap_write_ops *write_ops)
{
	struct iomap_iter iter = {
...
static inline int iomap_zero_iter_flush_and_stale(struct iomap_iter *i)
{
	...
	long end = i->pos + i->len - 1;

	i->iomap.flags |= IOMAP_F_STALE;
	return filemap_write_and_wait_range(mapping, i->pos, end);
}

static int iomap_zero_iter(struct iomap_iter *iter, int *did_zero,
		const struct iomap_write_ops *write_ops)
{
	unsigned long long bytes = iomap_length(iter);
	int status;
...
{
	struct address_space	*mapping = iter->inode->i_mapping;
	long			pstart = *start >> PAGE_SHIFT;
	long			pend = (end - 1) >> PAGE_SHIFT;
	unsigned int		count;
...
int
iomap_zero_range(struct inode *inode, long pos, long len, int *did_zero,
		const struct iomap_ops *ops,
		const struct iomap_write_ops *write_ops, void *private)
{
...
int
iomap_truncate_page(struct inode *inode, long pos, int *did_zero,
		const struct iomap_ops *ops,
		const struct iomap_write_ops *write_ops, void *private)
{
...
struct vm_fault_t iomap_page_mkwrite(struct vm_fault *vmf, const struct iomap_ops *ops,
		void *private)
{
	struct iomap_iter iter = {
...
static int iomap_writeback_range(struct iomap_writepage_ctx *wpc,
		struct folio *folio, unsigned long long pos, unsigned rlen, unsigned long long end_pos,
 ...
 */
static int iomap_writeback_handle_eof(struct folio *folio, struct inode *inode,
		unsigned long long *end_pos)
{
	unsigned long long isize = i_size_read(inode);

	if (*end_pos > isize) {
		size_t poff = offset_in_folio(folio, isize);
		long end_index = isize >> PAGE_SHIFT;
...
int iomap_writeback_folio(struct iomap_writepage_ctx *wpc, struct folio *folio)
{
	...
	unsigned long long end_pos = pos + folio_size(folio);
	unsigned long long end_aligned = 0;
	long orig_pos = pos;
	size_t bytes_submitted = 0;
	int error = 0;
	unsigned rlen;
