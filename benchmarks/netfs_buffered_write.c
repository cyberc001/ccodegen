...
static struct folio *netfs_grab_folio_for_write(struct address_space *mapping,
						long pos, size_t part)
{
	long index = pos / PAGE_SIZE;
	long fgp_flags = FGP_WRITEBEGIN;
...
void netfs_update_i_size(struct netfs_inode *ctx, struct inode *inode,
			 long pos, size_t copied)
{
	long i_size, end = pos + copied;
	long add;
	size_t gap;
 ...
 */
unsigned long long netfs_perform_write(struct kiocb *iocb, struct iov_iter *iter,
			    struct netfs_group *netfs_group)
{
	...
	unsigned long long written = 0, ret, ret2;
	long pos = iocb->ki_pos;
	size_t max_chunk = mapping_max_folio_size(mapping);
	int maybe_trouble = false;
 ...
 */
unsigned long long netfs_buffered_write_iter_locked(struct kiocb *iocb, struct iov_iter *from,
					 struct netfs_group *netfs_group)
{
	struct file *file = iocb->ki_filp;
 ...
 * * Number of bytes written, even for truncated writes
 */
unsigned long long netfs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	struct file *file = iocb->ki_filp;
 ...
 * we might otherwise.
 */
int netfs_page_mkwrite(struct vm_fault *vmf, struct netfs_group *netfs_group)
{
	struct netfs_group *group;
