...
DEFINE_MUTEX(delayed_uprobe_lock);
LIST_HEAD(delayed_uprobe_list);
 ...
 */
struct xol_area {
	struct wait_queue_head_t		wq;		/* if all slots are busy */
 ...
 *	  executable vma.
 */
static int valid_vma(struct vm_area_struct *vma, int is_register)
{
	struct vm_flags_t flags = VM_HUGETLB | VM_MAYEXEC | VM_MAYSHARE;
...
static unsigned long offset_to_vaddr(struct vm_area_struct *vma, long offset)
{
	return vma->vm_start + offset - ((long)vma->vm_pgoff << PAGE_SHIFT);
}

static long vaddr_to_offset(struct vm_area_struct *vma, unsigned long vaddr)
{
	return ((long)vma->vm_pgoff << PAGE_SHIFT) + (vaddr - vma->vm_start);
}
 ...
 */
int is_swbp_insn(struct uprobe_opcode_t *insn)
{
 ...
 */
int is_trap_insn(struct uprobe_opcode_t *insn)
{
...
static int verify_opcode(struct page *page, unsigned long vaddr, struct uprobe_opcode_t *insn,
			 int nbytes, void *data)
{
	struct uprobe_opcode_t old_opcode;
	int is_swbp;
...
static int valid_ref_ctr_vma(struct uprobe *uprobe,
...
static int orig_page_is_identical(struct vm_area_struct *vma,
		unsigned long vaddr, struct page *page, int *pmd_mappable)
{
	const long index = vaddr_to_offset(vma, vaddr) >> PAGE_SHIFT;
...
static int __uprobe_write(struct vm_area_struct *vma,
		...
		unsigned long insn_vaddr, struct uprobe_opcode_t *insn, int nbytes,
		int is_register)
{
...
int uprobe_write_opcode(struct arch_uprobe *auprobe, struct vm_area_struct *vma,
		const unsigned long opcode_vaddr, struct uprobe_opcode_t opcode,
		int is_register)
{
...
int uprobe_write(struct arch_uprobe *auprobe, struct vm_area_struct *vma,
		 const unsigned long insn_vaddr, struct uprobe_opcode_t *insn, int nbytes,
		 struct uprobe_write_verify_t verify, int is_register, int do_update_ref_ctr,
		 void *data)
{
	const unsigned long vaddr = insn_vaddr & PAGE_MASK;
 ...
 * Return 0 (success) or a negative errno.
 */
int set_orig_insn(struct arch_uprobe *auprobe,
		struct vm_area_struct *vma, unsigned long vaddr)
{
	return uprobe_write_opcode(auprobe, vma, vaddr,
			*(struct uprobe_opcode_t *)&auprobe->insn, false);
}
...
static inline int uprobe_is_active(struct uprobe *uprobe)
{
 ...
 */
static struct uprobe *hprobe_expire(struct hprobe *hprobe, int get)
{
...
static 
int uprobe_cmp(const struct inode *l_inode, const long l_offset,
 ...
 */
static struct uprobe *find_uprobe_rcu(struct inode *inode, long offset)
{
...
static struct uprobe *alloc_uprobe(struct inode *inode, long offset,
				   long ref_ctr_offset)
{
...
{
	static struct atomic64_t id;

	down_write(&uprobe->consumer_rwsem);
	list_add_rcu(&uc->cons_node, &uprobe->consumers);
	uc->id = (unsigned long long) atomic64_inc_return(&id);
...
static int __copy_insn(struct address_space *mapping, struct file *filp,
			void *insn, int nbytes, long offset)
{
	struct page *page;
	/*
				...
				struct mm_struct *mm, unsigned long vaddr)
{
	...
	ret = -ENOTSUPP;
	if (is_trap_insn((struct uprobe_opcode_t *)&uprobe->arch.insn))
		goto out;
...
static inline int consumer_filter(struct uprobe_consumer *uc, struct mm_struct *mm)
{
...
struct uprobe *uprobe_register(struct inode *inode,
				long offset, long ref_ctr_offset,
...
find_node_in_range(struct inode *inode, long min, long max)
{
...
static struct vm_fault_t xol_fault(const struct vm_special_mapping *sm,
...
{
	static struct uprobe_opcode_t insn = UPROBE_SWBP_INSN;
 ...
 */
static int xol_get_insn_slot(struct uprobe *uprobe, struct uprobe_task *utask)
{
...
static void free_ret_instance(struct uprobe_task *utask,
			      struct return_instance *ri, int cleanup_hprobe)
{
	unsigned seq;
 ...
 */
void uprobe_copy_process(struct task_struct *t, unsigned long longflags)
{
	struct uprobe_task *utask = current->utask;
	struct mm_struct *mm = current->mm;
	struct xol_area *area;
...
static void cleanup_return_instances(struct uprobe_task *utask, int chained,
					struct pt_regs *regs)
{
...
static int is_trap_at_addr(struct mm_struct *mm, unsigned long vaddr)
{
	...
	pagefault_disable();
	result = __get_user(opcode, (struct uprobe_opcode_t*)vaddr);
	pagefault_enable();
...
static struct uprobe *find_active_uprobe_speculative(unsigned long bp_vaddr)
{
	...
	offset = (long)(vma->vm_pgoff << PAGE_SHIFT) + (bp_vaddr - vma->vm_start);
...
static struct uprobe *find_active_uprobe_rcu(unsigned long bp_vaddr, int *is_swbp)
{
	...
	if (vma) {
		if (vma->vm_file) {
			...
			long offset = vaddr_to_offset(vma, bp_vaddr);
...
static struct return_instance *push_consumer(struct return_instance *ri, unsigned long longid, unsigned long long cookie)
{
...
static int ignore_ret_handler(int rc)
{
...
int arch_uprobe_ignore(struct arch_uprobe *aup, struct pt_regs *regs)
{
	return false;
}

int arch_uretprobe_is_alive(struct return_instance *ret, enum rp_check ctx,
					struct pt_regs *regs)
{
	return true;
}
